...
export const CloudflarePlugin: Plugin = async () => {
	return {
		config: async (input) => {
			if (!input.agent) {
				input.agent = {}
...
async function generateSystemPrompt(): Promise<string> {
	...
	const res = await fetch('https://developers.cloudflare.com/workers/prompt.txt')
	if (!res.ok) {
	...
	let systemPrompt = prompt.replace(/<user_prompt>.*<\/user_prompt>/s, '').trim()
	if (configFileInfo) {
